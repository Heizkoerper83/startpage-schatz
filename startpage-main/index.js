...
function applyFontFamily(name) {
  ...
  const fontLinkId = "selectedFontStylesheet";
  const oldFontLink = document.getElementById(fontLinkId);
  if (oldFontLink) oldFontLink.remove();

  if (genericFontFamilies.has(fontName.toLowerCase())) {
    document.documentElement.style.setProperty("--font-family", fontName.toLowerCase());
    return;
  }

  // Quote the entire name so CSS punctuation cannot change other style rules.
  const quotedName = '"' + fontName.replace(/\\/g, "\\\\").replace(/"/g, '\\"') + '"';
  const fallback = fontName === defaultFontFamily ? "monospace" : "sans-serif";
  document.documentElement.style.setProperty("--font-family", quotedName + ", " + fallback);
...
const savedFontFamily = localStorage.getItem("font-family") || defaultFontFamily;
fontFamilyInput.value = savedFontFamily;
applyFontFamily(savedFontFamily);
fontFamilyInput.addEventListener("change", function () {
  const fontName = this.value.trim() || defaultFontFamily;
  this.value = fontName;
  localStorage.setItem("font-family", fontName);
...
function updateEasyTabsSettings(TabstoUpdate) {
  ...
  $("#easyTabsInputHolder")
    ...
    .each(function (index, element) {
      ...
      colorPickerTabs.value = colorToHex(TabstoUpdate[index].color);
...
if (easyTabsArray) {
  ...
  easyTabsArray = defaultTabs;
}

// Only colors chosen with the settings picker override colors in defaultTabs.
// Migrate colors stored by older versions once, keeping custom picker choices.
const savedColorOverrides = localStorage.getItem("tabColorOverrides");
const tabColorOverrides = savedColorOverrides ? JSON.parse(savedColorOverrides) : {};
if (savedColorOverrides === null) {
  ...
  easyTabsArray.forEach((tab, index) => {
    const previousPickerColor = localStorage.getItem(`column${index + 1}c`);
    const savedColor = colorToHex(tab.color);
    if (previousPickerColor && !formerDefaults[index].includes(colorToHex(previousPickerColor))) {
      tabColorOverrides[index] = previousPickerColor;
    } else if (
      savedColor !== colorToHex(defaultTabs[index].color) &&
      !formerDefaults[index].includes(savedColor)
    ) {
      tabColorOverrides[index] = tab.color;
    }
  });
  localStorage.setItem("tabColorOverrides", JSON.stringify(tabColorOverrides));
}

if (localStorage.getItem("generalGreenDefaultMigration") !== "done") {
  if (tabColorOverrides[1] && colorToHex(tabColorOverrides[1]) === "#ff8989") {
    delete tabColorOverrides[1];
    localStorage.setItem("tabColorOverrides", JSON.stringify(tabColorOverrides));
  }
  localStorage.setItem("generalGreenDefaultMigration", "done");
}

easyTabsArray.forEach((tab, index) => {
  tab.color = tabColorOverrides[index] || defaultTabs[index].color;
  localStorage.removeItem(`column${index + 1}c`);
});
localStorage.setItem("easyTabsArray", JSON.stringify(easyTabsArray));
updateEasyTabs(easyTabsArray);
updateEasyTabsSettings(easyTabsArray);
...
// Keep the picker, displayed heading, and saved tab color in sync.
function colorToHex(color) {
  if (color.startsWith("#")) return color;
  const channels = color.match(/\d+/g);
  return channels && channels.length >= 3
    ? "#" + channels.slice(0, 3).map((value) => Number(value).toString(16).padStart(2, "0")).join("")
    : "#000000";
}

easyTabsArray.forEach((tab, index) => {
  const picker = document.getElementById(`colorcolumn${index + 1}`);
  if (!picker) return;
  picker.value = colorToHex(tab.color);
});

$("#easyTabsInputHolder").on("input", "input[type=color]", function () {
  const index = Number(this.id.replace("colorcolumn", "")) - 1;
  easyTabsArray[index].color = this.value;
  tabColorOverrides[index] = this.value;
  localStorage.setItem("tabColorOverrides", JSON.stringify(tabColorOverrides));
  localStorage.setItem("easyTabsArray", JSON.stringify(easyTabsArray));
  updateEasyTabs(easyTabsArray);
  this.parentElement.querySelector("li.medium").style.color = this.value;
});
